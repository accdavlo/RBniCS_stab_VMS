...
if linear_implicit:
    b_form = 0.5*(inner(dot(u_prev,grad(u)),v)  + inner(dot(u_prev,grad(v)),u) )*dx
    s_conv = (tau_v*inner(sigma_star(dot(u_prev,grad(u))),sigma_star(dot(u_prev,grad(v))))) *dx

else:
    b_form = 0.5*(inner(dot(u,grad(u)),v)  + inner(dot(u,grad(v)),u) )*dx
    s_conv = (tau_v*inner(sigma_star(dot(u,grad(u))),sigma_star(dot(u,grad(v))))) *dx
